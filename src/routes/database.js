...
router.post('/migrate', async (req, res) => {
  try {
    ...
    // Execute the complete database setup migration step by step
    ...
    for (const query of enumQueries) {
      try {
        ...
        // Ignore errors if enum already exists
        if (!error.message.includes('already exists')) {
          console.log('Enum creation warning:', error.message);
        }
      }
    }
    ...
    for (const query of fkQueries) {
      try {
        ...
        if (!error.message.includes('already exists')) {
          console.log('FK constraint warning:', error.message);
        }
      }
    }
